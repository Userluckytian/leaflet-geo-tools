...
import { HistoryStack, SnappingManager } from "../core/EditorCore";
...
export abstract class BaseEditor {
    ...
    // 历史堆栈（用于撤销/重做）
    protected history: HistoryStack<any>;
    ...
    // #region 历史/撤销相关（简单封装，子类可重写 restoreState）

    /** 将当前状态推入历史栈；建议子类在重要修改点调用此方法 */
    public pushHistory(state: any): void {
        this.history.push(state);
    }

    /** 撤销并尝试恢复状态（子类应实现 restoreState） */
    public undo(): any | null {
        const s = this.history.undo();
        if (s) this.restoreState(s);
        return s;
    }

    /** 重做并尝试恢复状态（子类应实现 restoreState） */
    ...
    /** 恢复状态的占位实现，子类可覆盖以按需恢复编辑器状态 */
    protected restoreState(_state: any): void {
        // no-op: override in specific editors to apply geometry/state
    }
